...
export default class AwayLockExtension extends Extension {
    ...
    _maybeScheduleLock() {
        ...
        if (!this._getConfiguredAddress()) {
            this._clearLockTimeout();
            this._publishStatus('no-device-configured');
            return;
        }
    ...
    _lock() {
        try {
            ...
            logError(error, 'awaylock: ScreenSaver.Lock failed, falling back to loginctl');
